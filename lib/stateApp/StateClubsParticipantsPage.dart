...
import 'package:sport_ims/models/DistrictModel.dart';
import '../models/ClubsModel.dart';
import '../models/EventModel.dart';
import '../models/EventParticipantsModel.dart';
import '../models/UsersModel.dart';

class StateClubsParticipantsPage extends StatefulWidget {
  final String stateName;

  StateClubsParticipantsPage({ required this.stateName});

  @override
  _StateClubsParticipantsPageState createState() => _StateClubsParticipantsPageState();
}

class _StateClubsParticipantsPageState extends State<StateClubsParticipantsPage> {
  List<Club> clubs = [];
  ...
  Future<void> fetchClubs() async {
    ...
    if (snapshot.exists) {
      for (final child in snapshot.children) {
        ...
        if (club.state == widget.stateName && club.approval == 'Approved') {
          clubList.add(club);
        }
      }
    }
  ...
  // Filter the number of participants in each club for the selected event
  // void filterParticipantsByDistrict(district) {
  //   if (district != null) {
  //     setState(() {
  //       clubParticipantsCount = clubs.map((club) {
  //         int participantsCount = 0;
  ...
  //         // Count participants in the club
  //         participantsCount = evenParticipants.where((id) => id.club==club.district && id.eventID==selectedEventModel.id).length;
  ...
  void _searchClubs(String value) {
    setState(() {
            tableData = clubParticipantsCount.where((club) => club.clubName.toLowerCase().contains(value.toLowerCase()) || club.district.toLowerCase().contains(value.toLowerCase()) || club.contactNumber.toLowerCase().contains(value.toLowerCase())  ).toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('State Club Participants Report', style: TextStyle(color: Colors.white)),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: _buildDropdown(
                  value: Constants().districts.where((e)=>e.state==widget.stateName).toList().first.name,
                  hint: 'Select District',
                  items: () {
                    var districts = [District(name: "ALL", code: "", state: "")];
                    var fetchedDisrticts=Constants().districts.where((e)=>e.state==widget.stateName).toList();
                    districts.addAll(fetchedDisrticts);
  ...
  Future<void> exportToExcel() async {
    try {
      ...
      final anchor = html.AnchorElement(
        ...
        ..setAttribute('download', '${widget.stateName}_${selectedEvent}_ClubsParticipantsData.xlsx')
        ..click();
