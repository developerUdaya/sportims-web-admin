...
class StateClubsSkatersPage extends StatefulWidget {
  final String stateName;

  StateClubsSkatersPage({required this.stateName});

  @override
  _DistrictClubsSkatersPageState createState() => _DistrictClubsSkatersPageState();
}

class _DistrictClubsSkatersPageState extends State<StateClubsSkatersPage> {
  List<Club> clubs = [];
    ...
    if (clubsSnapshot.exists) {
      for (final child in clubsSnapshot.children) {
        ...
        if (club.state == widget.stateName && club.approval == 'Approved') {
          ...
          print("Club name ${club.state}  == ${widget.stateName}");
          clubList.add(club);
        }
      }
    }
  ...
  void _searchClubs(String value) {
    setState(() {
      tableData = clubSkatersCount.where((club) => club.clubName.toLowerCase().contains(value.toLowerCase()) || club.district.toLowerCase().contains(value.toLowerCase()) || club.contactNumber.toLowerCase().contains(value.toLowerCase())  ).toList();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('State Club Report', style: TextStyle(color: Colors.white)),
  ...
  Future<void> exportToExcel() async {
    try {
      ...
      final anchor = html.AnchorElement(
        ...
        ..setAttribute('download', '${widget.stateName}_ClubsSkatersData.xlsx')
        ..click();
